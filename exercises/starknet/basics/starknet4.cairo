...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        // TODO: only owner can call this - takes product and new_stock - adds new_stock to stock in inventory
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            self.only_owner();
            let current_stock = self.storage_inventory.read(product);
            self.storage_inventory.write(product, current_stock + new_stock);
        }

        // TODO: takes product and quantity - subtracts quantity from stock in inventory - anybody can call this
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            let quantity_in_stock = self.storage_inventory.read(product);
            self.storage_inventory.write(product, quantity_in_stock - quantity);
        }

        // TODO: - takes product - returns product stock in inventory
    ...
    impl PrivateMethods of PrivateMethodsTrait {
        fn only_owner(self: @ContractState) {
            ...
            assert(caller == self.get_owner(), 'only owner can call this fn')
        }
    }
}
...
mod test {
    ...
    fn deploy_contract() -> ILizInventoryDispatcher {
        ...
        let (address0, _) = deploy_syscall(LizInventory::TEST_CLASS_HASH.try_into().unwrap(), 0, calldata.span(), false).unwrap();
