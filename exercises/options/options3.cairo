...
fn display_grades(student: @Student, index: usize) {
    // don't mind these lines! They are required when running recursive functions.
    ...
    // TODO: Modify the following lines so that if there is a grade for the course, it is printed. Otherwise, print "No grade".
    if course.is_some() {
        course.unwrap().print();
    }
